...
//            dToClosest = enemyArchon4.distanceSquaredTo(from);
    ...
    /**
     * replaces all instances of the provided location with the next closest archon loc
     *    ASSUMES archon locs are already read from comms
     * @param archonToReplace the archon to replace
     * @return the archon location which was used as replacement
     * @throws GameActionException if writing fails
     */
    public MapLocation replaceOurArchon(MapLocation archonToReplace) throws GameActionException {
      MapLocation closestFriendlyArchon = null;
      int dToClosest = 9999;
      boolean sameLoc4 = false;
      boolean sameLoc3 = false;
      boolean sameLoc2 = false;
      boolean sameLoc1 = false;
      switch (Global.rc.getArchonCount()) {
        case 4:
          if (ourArchon4.equals(archonToReplace)) {
            sameLoc4 = true;
          } else if (ourArchon4.isWithinDistanceSquared(archonToReplace, dToClosest-1)) {
            closestFriendlyArchon = ourArchon4;
            dToClosest = ourArchon4.distanceSquaredTo(archonToReplace);
          }
        case 3:
          if (ourArchon3.equals(archonToReplace)) {
            sameLoc3 = true;
          } else if (ourArchon3.isWithinDistanceSquared(archonToReplace, dToClosest-1)) {
            closestFriendlyArchon = ourArchon3;
            dToClosest = ourArchon3.distanceSquaredTo(archonToReplace);
          }
        case 2:
          if (ourArchon2.equals(archonToReplace)) {
            sameLoc2 = true;
          } else if (ourArchon2.isWithinDistanceSquared(archonToReplace, dToClosest-1)) {
            closestFriendlyArchon = ourArchon2;
            dToClosest = ourArchon2.distanceSquaredTo(archonToReplace);
          }
        case 1:
          if (ourArchon1.equals(archonToReplace)) {
            sameLoc1 = true;
          } else if (ourArchon1.isWithinDistanceSquared(archonToReplace, dToClosest-1)) {
            closestFriendlyArchon = ourArchon1;
//            dToClosest = enemyArchon4.distanceSquaredTo(from);
          }
      }
      if (closestFriendlyArchon == null) {
        switch (Global.rc.getArchonCount()) {
          case 4:
          case 3:
            closestFriendlyArchon = ourArchon4;
...
//      Printer.print("replaced WITH: " + closestFriendlyArchon);
    ...
    public MapLocation replaceEnemyArchon(MapLocation archonToReplace) throws GameActionException {
      MapLocation closestEnemyArchon = null;
      int dToClosest = 9999;
      boolean sameLoc4 = false;
      boolean sameLoc3 = false;
      ...
      switch (Global.rc.getArchonCount()) {
        case 4:
          if (enemyArchon4.equals(archonToReplace)) {
            sameLoc4 = true;
          } else if (enemyArchon4.isWithinDistanceSquared(archonToReplace, dToClosest-1)) {
            closestEnemyArchon = enemyArchon4;
            dToClosest = enemyArchon4.distanceSquaredTo(archonToReplace);
          }
        case 3:
          if (enemyArchon3.equals(archonToReplace)) {
            sameLoc3 = true;
          } else if (enemyArchon3.isWithinDistanceSquared(archonToReplace, dToClosest-1)) {
            closestEnemyArchon = enemyArchon3;
            dToClosest = enemyArchon3.distanceSquaredTo(archonToReplace);
          }
        case 2:
          if (enemyArchon2.equals(archonToReplace)) {
            sameLoc2 = true;
          } else if (enemyArchon2.isWithinDistanceSquared(archonToReplace, dToClosest-1)) {
            closestEnemyArchon = enemyArchon2;
            dToClosest = enemyArchon2.distanceSquaredTo(archonToReplace);
          }
        case 1:
          if (enemyArchon1.equals(archonToReplace)) {
            sameLoc1 = true;
          } else if (enemyArchon1.isWithinDistanceSquared(archonToReplace, dToClosest-1)) {
            closestEnemyArchon = enemyArchon1;
//            dToClosest = enemyArchon4.distanceSquaredTo(from);
          }
      }
      ...
      if (sameLoc4) {
//        Printer.print("Replacing archon4: " + enemyArchon4);
        enemyArchon4 = closestEnemyArchon;
        setEnemyArchonLoc(4, closestEnemyArchon);
      }
      if (sameLoc3) {
//        Printer.print("Replacing archon3: " + enemyArchon3);
        enemyArchon3 = closestEnemyArchon;
        setEnemyArchonLoc(3, closestEnemyArchon);
      }
      if (sameLoc2) {
//        Printer.print("Replacing archon2: " + enemyArchon2);
        enemyArchon2 = closestEnemyArchon;
        setEnemyArchonLoc(2, closestEnemyArchon);
      }
      if (sameLoc1) {
//        Printer.print("Replacing archon1: " + enemyArchon1);
        enemyArchon1 = closestEnemyArchon;
        setEnemyArchonLoc(1, closestEnemyArchon);
      }
//      Printer.print("replaced WITH: " + closestFriendlyArchon);
      return closestEnemyArchon;
    }
  ...
  public static class SpawnInfo {
    public static final int NUM_SPAWN_INTS = 2;
    ...
    public static final int ROUND_MOD_TWO_BIT_MASK = 0b1 << 15;
    public static final int NUM_MINERS_NEEDED_MASK = ~ROUND_MOD_TWO_BIT_MASK;
    public static final int NUM_MINERS_ALIVE_IND = SPAWN_INTS_START+1;
    public static final int NUM_MINERS_LIVE_COUNT_MASK = 0b11111111;
    public static final int NUM_MINERS_LIVE_COUNT_INVERTED_MASK = ~NUM_MINERS_LIVE_COUNT_MASK;
    public static final int SHIFTED_MINER_INCREMENT = 0b1 << 8;
    public static final int SHIFTED_MINER_INCREMENT_PLUS_1 = SHIFTED_MINER_INCREMENT + 1;

    public int getNumMinersNeeded() throws GameActionException {
      return Global.rc.readSharedArray(NUM_MINERS_NEEDED_IND) & NUM_MINERS_NEEDED_MASK;
    }
    ...
    public void incrNumMinersNeeded(int amt) throws GameActionException {
      Global.rc.writeSharedArray(NUM_MINERS_NEEDED_IND, Global.rc.readSharedArray(NUM_MINERS_NEEDED_IND) + amt);
    }

    public boolean checkFirstArchonToUseArchonInfo() throws GameActionException {
      int val = Global.rc.readSharedArray(NUM_MINERS_NEEDED_IND);
      if ((val & ROUND_MOD_TWO_BIT_MASK) >> 15 != Cache.PerTurn.ROUND_NUM % 2) {
        Global.rc.writeSharedArray(NUM_MINERS_NEEDED_IND, val ^ ROUND_MOD_TWO_BIT_MASK);
        Global.rc.writeSharedArray(NUM_MINERS_ALIVE_IND, (Global.rc.readSharedArray(NUM_MINERS_ALIVE_IND) & NUM_MINERS_LIVE_COUNT_MASK) << 8);
        return true;
      }
      return false;
    }
    ...
    public void setNumMinersNeeded(int newNum) throws GameActionException {
      Global.rc.writeSharedArray(NUM_MINERS_NEEDED_IND, (Global.rc.readSharedArray(NUM_MINERS_NEEDED_IND) & ROUND_MOD_TWO_BIT_MASK) + newNum);
    }

    public void incrMinerLiveCount() throws GameActionException {
      Global.rc.writeSharedArray(NUM_MINERS_ALIVE_IND, Global.rc.readSharedArray(NUM_MINERS_ALIVE_IND) + 1);
//      Printer.print("increment live counter");
    }

    public void incrMinerMainCount() throws GameActionException {
      Global.rc.writeSharedArray(NUM_MINERS_ALIVE_IND, Global.rc.readSharedArray(NUM_MINERS_ALIVE_IND) + SHIFTED_MINER_INCREMENT);
    }

    public void incrMinerMainAndLiveCount() throws GameActionException {
      Global.rc.writeSharedArray(NUM_MINERS_ALIVE_IND, Global.rc.readSharedArray(NUM_MINERS_ALIVE_IND) + SHIFTED_MINER_INCREMENT_PLUS_1);
    }

    public int getMinerLiveCount() throws GameActionException {
      return (Global.rc.readSharedArray(NUM_MINERS_ALIVE_IND) & NUM_MINERS_LIVE_COUNT_MASK);
    }
    public int getMinerMainCount() throws GameActionException {
      return (Global.rc.readSharedArray(NUM_MINERS_ALIVE_IND) & NUM_MINERS_LIVE_COUNT_INVERTED_MASK) >> 8;
    }
...
//  public final ChunkInfo chunkInfo;
  ...
  public final SpawnInfo spawnInfo;
