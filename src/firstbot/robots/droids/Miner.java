...
public class Miner extends Droid {
  ...
  private final int RAD_SQ_TO_RUN_FROM_COMM;
  ...
  public Miner(RobotController rc) throws GameActionException {
    ...
    RAD_SQ_TO_RUN_FROM_COMM = Cache.Permanent.MAP_AREA < 1000 ? 40 : 70;
...
//    Printer.print("before incr: " + Clock.getBytecodeNum());
    communicator.spawnInfo.incrMinerLiveCount();
//    Printer.print("after incr: " + Clock.getBytecodeNum());
...
//    rc.setIndicatorLine(Cache.PerTurn.CURRENT_LOCATION, responseLocation, 0,255,0);
  ...
  private void mineSurroundingGold() throws GameActionException {
    ...
    for (MapLocation toMine : rc.senseNearbyLocationsWithGold(Cache.Permanent.ACTION_RADIUS_SQUARED)) {
  ...
  private void mineSurroundingLead() throws GameActionException {
    ...
    if (communicator.metaInfo.knownSymmetry != null) {
      ...
      if (Utils.maxSingleAxisDist(Cache.PerTurn.CURRENT_LOCATION, Cache.Permanent.START_LOCATION) < 7) leadToLeave = 1;
    }
...
//    System.out.println("Miner start followLead(" + Clock.getBytecodeNum() + ") - " + Cache.PerTurn.ROUND_NUM);
...
//    }
    ...
    if (highLead != null && !(closestCommedEnemy != null && !rc.canSenseLocation(closestCommedEnemy) && closestCommedEnemy.isWithinDistanceSquared(highLead, RAD_SQ_TO_RUN_FROM_COMM)) && (highLead.equals(Cache.PerTurn.CURRENT_LOCATION) || moveOptimalTowards(highLead))) {
...
//    System.out.println("high lead not found or blocked ");
  ...
  private static final int MAX_LEAD_LOCS_LEN = 5;
  /**
...
//    MapLocation[] leadLocs = rc.senseNearbyLocationsWithLead(Utils.DSQ_1by1, 1);
//    if (leadLocs.length >= 5 && rc.senseRubble(Cache.PerTurn.CURRENT_LOCATION) < 5) return Cache.PerTurn.CURRENT_LOCATION;
