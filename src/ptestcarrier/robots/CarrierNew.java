package ptestcarrier.robots;

import ptestcarrier.communications.CommsHandler;
import ptestcarrier.communications.Communicator;
import ptestcarrier.communications.HqMetaInfo;
import ptestcarrier.containers.HashMap;
import ptestcarrier.containers.HashSet;
import ptestcarrier.robots.micro.CarrierWellPathing;
import ptestcarrier.knowledge.Cache;
import ptestcarrier.robots.pathfinding.BugNav;
import ptestcarrier.utils.Printer;
import ptestcarrier.utils.Utils;
...
public class CarrierNew extends MobileRobot {
    ...
    * if HQ gave no role, rng (no need to implement)
    ...
    private void readHQTask() throws GameActionException {
        ...
        int readThrough = CommsHandler.PRANAY_OUR_HQ_SLOTS;
...
//        if (approachWell(wellMiningFrom)) return;
    ...
    //todo: if exploration doesnt work, try to use known wells instead as target suggestions
...
//            if (rc.getID() == 11883) Printer.print("writing well info", "well=" + locWrote, " data=" + temporaryWellDataMap.get(locWrote));
    ...
    private void moveToTargetWell() throws GameActionException {
        ...
        if (Cache.PerTurn.CURRENT_LOCATION.isWithinDistanceSquared(wellMiningFrom, 16)) {
            ++numRoundsCloseToTarget;
        }

        // tried for 10 rounds, give up
//        if (rc.getID() == 13074) {
//            Printer.print("close to target well", "close=" + closeToTargetWell, "rounds=" + (Cache.PerTurn.ROUND_NUM - roundNumWhenCloseToTarget), "numRounds=" + numRoundsCloseToTarget);
//        }
        // if I can see all 3x3 spots around well and every spot is taken, this well is full and set to null

        MapLocation[] allMiningLocations = rc.getAllLocationsWithinRadiusSquared(wellMiningFrom, 2);
        int numMiners = 0;
        int numSensedLocs = 0;
        int numGoodLocations = 0;
        for (MapLocation loc : allMiningLocations) {
...
//        if (rc.getID() == 11193) {
//            Printer.print("close=" + closeToTargetWell, "rounds=" + (Cache.PerTurn.ROUND_NUM - roundNumWhenCloseToTarget), "numRounds=" + numRoundsCloseToTarget, " numSensedLocs=" + numSensedLocs + " numMiners=" + numMiners + " numGoodLocations=" + numGoodLocations);
//        }

        if (closeToTargetWell &&
                ((Cache.PerTurn.ROUND_NUM - roundNumWhenCloseToTarget >= 20 && numRoundsCloseToTarget >= 10)
                        || (Cache.PerTurn.ROUND_NUM - roundNumWhenCloseToTarget >= 50)
                        || ((numSensedLocs == 9 && numMiners >= numGoodLocations && numRoundsCloseToTarget >= 4)))) {
            closeToTargetWell = false;
            roundNumWhenCloseToTarget = 2000;
            numRoundsCloseToTarget = 0;
        ...
        if (Cache.PerTurn.ROUNDS_ALIVE == 0) {
            boolean y = pathing.moveInDirLoose(Cache.PerTurn.CURRENT_LOCATION.directionTo(wellMiningFrom));
        } else {
