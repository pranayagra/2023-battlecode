...
public class Archon extends Building {
  ...
  public static final int MIN_DIST_TO_ENEMY = 8;
  public static final int MAX_DIST_TO_ENEMY = 9;

  public static final int CRITICAL_HEALTH_TO_HEAL_LOWEST = 25;
...
//  private MapSymmetry predictedSymmetry;
  ...
  int initialSoldiersToSpawn;
  int totalSoldiersToSpawn;
  private boolean minersDone;
  private boolean soldiersDone;
...
//    Printer.print("num miners in live count: " + communicator.spawnInfo.getMinerLiveCount());
//    Printer.print("num miners in main count: " + communicator.spawnInfo.getMinerMainCount());
    if (communicator.spawnInfo.checkFirstArchonToUseArchonInfo()) {
      // live count gets shifted to main
//      Printer.print("Archon #" + whichArchonAmI + ": first run");
    }
//    Printer.print("num miners in live count: " + communicator.spawnInfo.getMinerLiveCount());
//    Printer.print("num miners in main count: " + communicator.spawnInfo.getMinerMainCount());
      ...
      if (saveUpForBuilderAndLab) {
        ...
        if (typeToSpawn != null && (rc.getTeamLeadAmount(Cache.Permanent.OUR_TEAM) >= typeToSpawn.buildCostLead * 2 || rc.getRoundNum() % archons == whichArchonAmI % archons || whichArchonAmI == archons)) {
          if (spawnDroid(typeToSpawn)) {
            if (initialMinersToSpawn == minersSpawned && typeToSpawn == RobotType.MINER) {
              minersDone = true;
              if (soldiersDone) saveUpForBuilderAndLab = true;
            }
            if (initialSoldiersToSpawn == soldiersSpawned && typeToSpawn == RobotType.SOLDIER) {
              soldiersDone = true;
              if (minersDone) saveUpForBuilderAndLab = true;
            }
          }
        }
      }
...
//    Printer.cleanPrint();;
    if (rc.canTransform() && closestCommedEnemy != null && (bestArchonToSpawnBuilderForLab != whichArchonAmI || labBuilderSpawned)) {
...
//      updateShouldStop();
      if (shouldStart() && rc.getArchonCount() > 1) {
...
//      updateShouldStop();
      ...
      if (!shouldStop()) {
        if (Utils.maxSingleAxisDist(whereToGo, Cache.PerTurn.CURRENT_LOCATION) < MAX_DIST_TO_ENEMY ? moveOptimalAway(whereToGo) : moveOptimalTowards(whereToGo)) {
  ...
  public boolean shouldStart() throws GameActionException {
...
//    if (whereToGo != null && whereToGo.isWithinDistanceSquared(Cache.PerTurn.CURRENT_LOCATION, Cache.Permanent.ACTION_RADIUS_SQUARED)) {
    int distToClosestEnemy = 9999;
    if (closestCommedEnemy != null) {
      distToClosestEnemy = Utils.maxSingleAxisDist(closestCommedEnemy, Cache.PerTurn.CURRENT_LOCATION);
    }
    for (RobotInfo enemy : Cache.PerTurn.ALL_NEARBY_ENEMY_ROBOTS) {
      if (enemy.type.damage > 0 || enemy.type == RobotType.ARCHON) {
        distToClosestEnemy = Math.min(distToClosestEnemy, Utils.maxSingleAxisDist(enemy.location, Cache.PerTurn.CURRENT_LOCATION));
      }
    }
    if (distToClosestEnemy <= MIN_DIST_TO_ENEMY-3 || distToClosestEnemy >= MAX_DIST_TO_ENEMY+4) {
//      shouldStop = true;
      return true;
    }
...
//    shouldStop = false;
  ...
  public boolean shouldStop() throws GameActionException {
    // check if any enemy damaging units or any damaged friendly soldiers
//    shouldStop = false;
...
//    if (whereToGo != null && whereToGo.isWithinDistanceSquared(Cache.PerTurn.CURRENT_LOCATION, Cache.Permanent.ACTION_RADIUS_SQUARED)) {
    ...
    if (MIN_DIST_TO_ENEMY <= distToClosestEnemy && distToClosestEnemy <= MAX_DIST_TO_ENEMY) {
//      shouldStop = true;
      return true;
    }
...
//    }
  ...
  private void runTeamStartupLogic() throws GameActionException {
    ...
    // set default guesses for enemy archon position
    communicator.archonInfo.mirrorSelfToEnemies();

//    initialMinersToSpawn += totalMoves / 75;
    switch (rc.getArchonCount()) {
      case 1:
        initialMinersToSpawn = 4;
        break;
      case 2:
      case 3:
        initialMinersToSpawn = 2;
        break;
      case 4:
        initialMinersToSpawn = 1;
    }

    if (communicator.spawnInfo.checkFirstArchonToUseArchonInfo()) {
      communicator.spawnInfo.setNumMinersNeeded((initialMinersToSpawn + 1) * rc.getArchonCount());
    }

    if (Cache.Permanent.MAP_AREA <= 1000) {
      initialSoldiersToSpawn = initialMinersToSpawn-1;
      totalSoldiersToSpawn = initialSoldiersToSpawn+2;
    }
...
//      System.out.println("I need a miner! bestLead: " + bestLead + " dir: " + dir);
        if (buildRobotInDirLoose(RobotType.MINER, dir) || buildRobot(RobotType.MINER, getLeastRubbleUnoccupiedDir())) {
          rc.setIndicatorString("Spawn miner!");
          minersSpawned++;
          leadSpent += RobotType.MINER.buildCostLead;
          communicator.spawnInfo.incrMinerMainAndLiveCount();
          return true;
        }
        break;
      case BUILDER:
...
//    System.out.println(Cache.PerTurn.CURRENT_LOCATION + " --\nminersSpawned: " + minersSpawned + "\nrc.getArchonCount(): " + rc.getArchonCount() + "\nsoldiersSpawned: " + soldiersSpawned);
    ...
    return labBuilderSpawned
            ? communicator.spawnInfo.getMinerMainCount() < communicator.spawnInfo.getNumMinersNeeded()
            : minersSpawned < initialMinersToSpawn;
...
//    );
   ...
   */
  private boolean needBuilder() throws GameActionException {
//    return (Cache.Permanent.MAP_AREA >= 1000 || soldiersSpawned >= totalSoldiersToSpawn) && buildersSpawned < 70 && (
//        communicator.spawnInfo.getMinerMainCount() >= communicator.spawnInfo.getNumMinersNeeded()
//        || rc.getTeamLeadAmount(rc.getTeam()) > 200
...
//    if (Cache.Permanent.MAP_AREA <= 625) return soldiersSpawned < 3; // 3 soldiers
//    return labBuilderSpawned
//            ? soldiersSpawned < totalSoldiersToSpawn
//            : soldiersSpawned < initialSoldiersToSpawn;
    return false;// soldiersSpawned < 1;// || soldiersSpawned < sagesSpawned / 3;
  }
